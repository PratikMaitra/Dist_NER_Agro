...
def process_sentence(args):
    sentence, dic, tag, tagIdx, windowSize = args
    ...
    return sentence, labeled_word, count
...
def lookup_in_Dic(tag2idx, dicFile, sentences, tag, windowSize):
    ...
    args = [(sentence, dic, tag, tagIdx, windowSize) for sentence in sentences]
    num_cores = min(mp.cpu_count(), 8)
    with mp.Pool(num_cores) as pool:
        results = list(tqdm(pool.imap(process_sentence, args), total=len(args), desc="Parallel lookup"))

    updated_sentences, labeled_words_list, counts = zip(*results)
    labeled_words = set().union(*labeled_words_list)
    total_count = sum(counts)

    return list(updated_sentences), len(labeled_words), total_count
...
def main():
    ...
    testFile = f'./data/{dataset}/test.txt'
