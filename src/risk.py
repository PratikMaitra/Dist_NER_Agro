...
class Risk(object):
    ...
    def compute_risk(self, logits, labels, probs=None, risk_type=None):
        ...
        elif self.risk_type == 'Conf-MPU':# Correct one
            l_mask = []
            p_mask = []
        ...
        # Add this right at the end of the compute_risk method:
        if not isinstance(risk, torch.Tensor):
            risk = torch.tensor(risk, dtype=torch.float32, device=device, requires_grad=True)
        return risk
    ...
    def risk_on_val(self, logits, labels):
        ...
        return torch.tensor(risk, dtype=torch.float32, device=device) if not isinstance(risk, torch.Tensor) else risk
