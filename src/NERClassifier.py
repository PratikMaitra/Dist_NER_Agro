...
class NERClassifier(object):
    ...
    def train(self, model_idx=0):
        ...
        save_path = os.path.join(self.output_dir, f"model_{model_idx}")
        os.makedirs(save_path, exist_ok=True)
        self.save_model(model, f"model_{model_idx}.pt", save_path)
    ...
    def token_hardness_score(self, file_dir):
        ...
        if os.path.exists(os.path.join(self.temp_dir, f"hardness_score.np.npy")) : # Changed to not False for first time calculation
        ...
        for sentences in zip(*pred_prob_list):
            ...
            # Revert to original github        
            if len(scores) == 0:
                continue
            sent_score = np.array(scores).mean(axis=0)
            if sent_score.size != len(sentences[0]):
    ...
    def ensemble_pred(self, fild_dir):
        ...
        if len(pred_prob_list) == 0:
            print("❗ No prediction files found for ensemble. Skipping.")
            return
        ensemble_probs = []
    ...
    def curriculum_train(self):
        ...
        if not hasattr(self, "ensemble_label"):
            print("Generating ensemble predictions before curriculum training...")
            self.ensemble_pred(self.temp_dir)
    ...
    def predict_data(self, dt="test"):
        ...
        with open(write_file_name, "w") as fp:
            for sentence, pred in zip(train_sentences, y_pred):
                ...
                for s, p in zip(sentence, pred):
                    w = s[0]  # word
                    fp.writelines(f"{w} O {int('Trait' in p)}\n")
